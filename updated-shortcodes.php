...
function sjm_journals_shortcode_new($atts = array()) {
    ...
    // Enqueue the modern CSS files
    wp_enqueue_style('wjm-modern-admin', WJM_PLUGIN_URL . 'assets/css/wjm-modern-admin.css', array(), defined('WJM_VERSION') ? WJM_VERSION : '1.0.1');
    wp_enqueue_style('sjm-academic-shortcodes', plugin_dir_url(__FILE__) . 'academic-shortcodes.css', array('wjm-modern-admin'), defined('WJM_VERSION') ? WJM_VERSION : '1.0.1');
    ...
    // Page Header
    $output .= '<div class="sjm-page-header">';
    $output .= '<h1 class="sjm-page-title">Academic Journals</h1>';
    $output .= '<p class="sjm-page-subtitle">Discover peer-reviewed academic journals across all disciplines</p>';
    if (!empty($journals)) {
        $output .= '<div class="sjm-page-stats">';
        $output .= '<div class="sjm-stat-item"><span class="sjm-stat-number">' . count($all_journals) . '</span><span class="sjm-stat-label">Total Journals</span></div>';
        $output .= '<div class="sjm-stat-item"><span class="sjm-stat-number">' . count(array_filter($all_journals, function($j) { return get_post_meta($j->ID, '_sjm_open_access', true); })) . '</span><span class="sjm-stat-label">Open Access</span></div>';
        $output .= '<div class="sjm-stat-item"><span class="sjm-stat-number">' . count(array_filter($all_journals, function($j) { return get_post_meta($j->ID, '_sjm_peer_reviewed', true); })) . '</span><span class="sjm-stat-label">Peer Reviewed</span></div>';
        $output .= '</div>';
    }
    $output .= '</div>';

    // Fixed XSS vulnerability - sanitize all $_GET parameters before use
    $get_publisher = isset($_GET['publisher']) ? sanitize_text_field($_GET['publisher']) : '';
    $get_subject_area = isset($_GET['subject_area']) ? sanitize_text_field($_GET['subject_area']) : '';
    $get_language = isset($_GET['language']) ? sanitize_text_field($_GET['language']) : '';
    $get_open_access = isset($_GET['open_access']) ? sanitize_text_field($_GET['open_access']) : '';
    $get_peer_reviewed = isset($_GET['peer_reviewed']) ? sanitize_text_field($_GET['peer_reviewed']) : '';

    // Search filters for journals
    $output .= '<form method="get" class="sjm-filters">';
    $output .= '<div class="sjm-filters-header">';
    $output .= '<h2 class="sjm-filters-title"><svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="display:inline-block;vertical-align:middle;margin-right:8px;"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path></svg>Filter Journals</h2>';
    $output .= '<p class="sjm-filters-subtitle">Refine your search using the filters below</p>';
    $output .= '</div>';
    ...
    if (!empty($publishers)) {
        ...
        foreach ($publishers as $publisher) {
            $selected = ($get_publisher === $publisher) ? 'selected' : '';
    ...
    if (!empty($subject_areas)) {
        ...
        foreach ($subject_areas as $subject) {
            $selected = ($get_subject_area === $subject) ? 'selected' : '';
    ...
    if (!empty($languages)) {
        ...
        foreach ($languages as $language) {
            $selected = ($get_language === $language) ? 'selected' : '';
    ...
    $selected = ($get_open_access === '1') ? 'selected' : '';
    ...
    $selected = ($get_peer_reviewed === '1') ? 'selected' : '';
    ...
    // Handle URL parameters for filtering - Fixed XSS vulnerability
    if (isset($_GET['sjm_journal_filter'])) {
        // Use already sanitized values from above
        $filter_args = array(
            'post_type' => 'journal',
            'posts_per_page' => intval($atts['per_page']),
            'meta_query' => array('relation' => 'AND')
        ...
        if (!empty($get_subject_area)) {
            $filter_args['meta_query'][] = array(
                'key' => '_sjm_subject_areas',
                'value' => $get_subject_area,
                'compare' => 'LIKE'
            );
        }
