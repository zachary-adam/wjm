...
// Clean up any remaining transients (paper and issue validation notices)
global $wpdb;
$wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_sjm_paper_required_notice_%'");
$wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_sjm_issue_required_notice_%'");
